...
def _popen(cmd, data_in, data_out=PIPE):
    ...
    p = Popen(cmd, bufsize=-1, stdin=PIPE, stdout=data_out, stderr=PIPE,
              shell=shell)  # shell=True required for Linux? quick fix, but may be security concern
    ...
    def analysis1axis(self, trackerdict=None, singleindex=None, accuracy='low',
        ...
        for index in trackerkeys:  # either full list of trackerdict keys, or single index
            ...
            try:  # look for missing data
                ...
                analysis.analysis(octfile=octfile, name=name, frontscan=frontscanind, backscan=backscanind,
                                  accuracy=accuracy)
...
class MetObj:
    ...
    def __init__(self, tmydata, metadata, label='right'):
        ...
        try:  # make sure the data is tz-localized.
            datetimetz = datetimetz.tz_localize(
                pytz.FixedOffset(self.timezone * 60))  # use pytz.FixedOffset (in minutes)
            ...
            if interval == pd.Timedelta('1h'):
                ...
                if label.lower() == 'right':
                    ...
                    sunup['minutedelta'] = int(
                        interval.seconds / 2 / 60)  # default sun angle 30 minutes before timestamp
                    # vector update of minutedelta at sunrise
                    sunrisemask = sunup.index.hour - 1 == sunup['sunrise'].dt.hour
                    sunup['minutedelta'].mask(sunrisemask, np.floor((60 - (sunup['sunrise'].dt.minute)) / 2),
                                              inplace=True)
                    # vector update of minutedelta at sunset
                    sunsetmask = sunup.index.hour - 1 == sunup['sunset'].dt.hour
                    sunup['minutedelta'].mask(sunsetmask, np.floor((60 - (sunup['sunset'].dt.minute)) / 2),
                                              inplace=True)
...
class AnalysisObj:
    ...
    def _saveResultsCumulative(self, data, reardata=None, savefile=None):
        ...
        if reardata is not None:
            ...
            self.backRatio = [x / (y + .001) for x, y in
                              zip(reardata['Wm2'], data['Wm2'])]  # add 1mW/m2 to avoid dividebyzero
    ...
    def moduleAnalysis(self, scene, modWanted=None, rowWanted=None,
        ...
        if ((getattr(scene.module, 'cellModule', None)) and
            ...
            xinc_back = -((sceney - ycell) / (scene.module.cellModule.numcellsy - 1)) * np.cos((tilt) * dtor) * np.sin(
                (azimuth) * dtor)
            yinc_back = -((sceney - ycell) / (scene.module.cellModule.numcellsy - 1)) * np.cos((tilt) * dtor) * np.cos(
                (azimuth) * dtor)
            zinc_back = ((sceney - ycell) / (scene.module.cellModule.numcellsy - 1)) * np.sin(tilt * dtor)
            firstsensorxstartfront = xstartfront - scene.module.cellModule.ycell / 2 * np.cos((tilt) * dtor) * np.sin(
                (azimuth) * dtor)
